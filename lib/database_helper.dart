...
class DatabaseHelper {
  ...
  Future<List<Map<String, dynamic>>?> getTaskMapList(bool orderByDate, bool filterByStatus) async {
    Database? db = await this.db;
    dynamic query = await db?.query(taskTable);

    if (orderByDate && filterByStatus)
      query = await db?.query(taskTable, orderBy: '$colDate ASC', where: '$colStatus = 1');
    else if (orderByDate)
  ...
  Future<List<Task>> getTaskList(bool orderByDate, bool filterByStatus) async {
    final List<Map<String, dynamic>>? taskMapList = await getTaskMapList(orderByDate, filterByStatus);
  ...
  Future<int?> updateTask(Task task) async {
    Database? db = await this.db;
    final int? result = await db?.update(taskTable, task.toMap(), where: '$colId = ?', whereArgs: [
      task.id
    ]);
    return result;
  }
