...
class _TaskListSreenState extends State<TaskListScreen> {
  ...
  Widget _buildItem(Task task) {
    return Container(
      child: ListTile(
        ...
        title: Text(task.title!, style: TextStyle(decoration: task.status == 0 ? TextDecoration.none : TextDecoration.lineThrough)),
        subtitle: Text(_dateFormat.format(task.date), style: TextStyle(decoration: task.status == 0 ? TextDecoration.none : TextDecoration.lineThrough)),
        trailing: Checkbox(
  ...
  void initState() {
          ...
          builder: (context) => AlertDialog(
                ...
                actions: [
                  TextButton(
                    ...
                    child: Text('Dont Allow', style: TextStyle(color: Colors.green, fontSize: 18)),
                  ),
                  TextButton(
                    onPressed: () => AwesomeNotifications().requestPermissionToSendNotifications().then((_) => Navigator.pop(context)),
                    child: Text('Allow', style: TextStyle(color: Colors.green, fontSize: 18)),
                  )
                ],
              ));
    });
    ...
    AwesomeNotifications().actionStream.listen((event) {
      AwesomeNotifications().getGlobalBadgeCounter().then((value) => AwesomeNotifications().setGlobalBadgeCounter(value));
    });
  ...
  _updateTaskList() {
    setState(() {
      _taskList = DatabaseHelper.instance.getTaskList(orderByDate, filterByStatus);
                            ...
                            itemBuilder: (BuildContext context, int index) {
                              Future.delayed(Duration.zero, () async {
                                setState(() {
                                  ...
                                  completedTaskCount = snapshot.data.where((Task task) => task.status == 1).toList().length;
                                });
                              });
